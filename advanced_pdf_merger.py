...
# -*- coding: utf-8 -*-
"""
PDF Merger - Pokročilá aplikace pro spojování PDF souborů do dvoustrany
Autor: David Rynes
...
class AdvancedPDFMerger:
    """Pokročilá třída pro spojování PDF souborů do dvoustrany"""
    ...
    def pdf_to_image(self, pdf_path: Path, page_num: int = 0, dpi: int = 300) -> Image.Image:
        """Konvertuje PDF stránku na obrázek pomocí PyMuPDF"""
        try:
            doc = fitz.open(str(pdf_path))
            page = doc[page_num]
            
            # Nastavení DPI pro kvalitní výstup
            ...
            logger.error(f"Chyba při konverzi PDF na obrázek: {e}")
            return None
    ...
    def create_side_by_side_pdf(self, left_pdf: Path, right_pdf: Path, output_path: Path, 
        ...
        """
        try:
            # Konverze PDF souborů na obrázky
            logger.info("Konvertuji PDF soubory na obrázky...")
            left_img = self.pdf_to_image(left_pdf)
            right_img = self.pdf_to_image(right_pdf)
            
            if left_img is None or right_img is None:
                logger.error("Selhala konverze PDF na obrázky")
                return False
            
            # Získání rozměrů obrázků
            left_width, left_height = left_img.size
            right_width, right_height = right_img.size
            
            logger.info(f"Rozměry levého obrázku: {left_width} x {left_height}")
            logger.info(f"Rozměry pravého obrázku: {right_width} x {right_height}")
            
            # Vytvoření nového obrázku s dvojnásobnou šířkou
            ...
            combined_img.paste(right_img, (left_width, 0))
            
            # Rotace obrázku
            if rotation != 0:
                combined_img = combined_img.rotate(rotation, expand=True)
                logger.info(f"Obrázek otočen o {rotation} stupňů")
            
            # Vytvoření PDF z obrázku pomocí reportlab
            buffer = io.BytesIO()
            c = canvas.Canvas(buffer, pagesize=(combined_img.width, combined_img.height))
            ...
            c.drawImage(ImageReader(img_buffer), 0, 0, 
                       width=combined_img.width, height=combined_img.height)
            ...
            try:
                ...
                writer.add_metadata({
                    ...
                    '/Title': f'Spojené PDF: {left_pdf.stem} + {right_pdf.stem}',
                    '/Creator': 'PDF Merger v1.0',
                    '/Producer': 'PDF Merger v1.0'
                })
            except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Advanced PDF Merger - Spojování PDF souborů do dvoustrany")
    ...
    parser.add_argument("--dpi", type=int, default=300, help="DPI pro konverzi PDF na obrázky")
    
    args = parser.parse_args()
    
    # Vytvoření instance AdvancedPDFMerger
    merger = AdvancedPDFMerger(args.files_dir)
    ...
    if args.auto:
        ...
        print("\nPoužití:")
        print("  python advanced_pdf_merger.py --auto                    # Automatické spojení všech párových souborů")
        print("  python advanced_pdf_merger.py --left file1.pdf --right file2.pdf --output result.pdf")
        print("  python advanced_pdf_merger.py --rotation 90              # Rotace doprava místo doleva")
        print("  python advanced_pdf_merger.py --dpi 600                 # Vyšší kvalita (600 DPI)")
